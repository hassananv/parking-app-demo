...
from api.repository.auth_transactions import get_access_token
import base64
import hashlib
...
def token_user(request: Request, db: Session = Depends(get_db_session)):
    ...
    login_response = {"access_token": None, "token_type": "bearer", "login_url":getLoginUrl(request), "logout_url":getLogoutUrl(request), "keycloak":False}
    
    # print("________REFRESH__TOKEN______")
    login_response_token = get_access_token(request, db)
    if(login_response_token['access_token'] is not None):
        login_response['access_token'] = login_response_token['access_token']
    ...
    if("oidc_refresh_token" in request.session and request.session["oidc_refresh_token"] is not None):
        ...
        access_token = create_access_token(data={"sub": oidc_user.user.email, "username": oidc_user.user.username})
       
        return {"access_token": access_token, "token_type": "bearer", "login_url":None, "logout_url":getLogoutUrl(request), "keycloak":True}
    else:
        return login_response
